      ...
      <div className="flex space-x-4 mb-4">
        ...
        <div
          className={`flex flex-col gap-4 transition-all duration-300 ${
            isLeftPanelCollapsed ? "w-10 overflow-hidden" : "w-auto min-w-32"
          }`}
        >
          <Button
            onClick={() => setIsLeftPanelCollapsed(!isLeftPanelCollapsed)}
            size="icon"
            className="absolute left-2 top-12 z-999 p-2 rounded-full shadow-md"
          >
        ...
        <div
          id="right-panel"
          className={`flex flex-col gap-4 transition-all duration-300 ${
            isRightPanelCollapsed ? "w-10 overflow-hidden" : "w-auto min-w-32"
          }`}
        >
          <Button
          ...
          {
            // 没有数据时显示提示
            !isRightPanelCollapsed && !query.data?.drones && (
          ...
          {!isRightPanelCollapsed && query.data?.drones && (
            <div className="text-xl pt-3">实时数据</div>
          )}
          {!isRightPanelCollapsed &&
            query.data?.drones &&
