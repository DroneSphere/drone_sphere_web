...
export function DirectionsScaleControl({
  ...
  const [workingMode, setWorkingMode] = useState<string>("manual");
  ...
  // 根据方向更新云台角度
  const updateGimbalPosition = useCallback(
    (direction: Direction, step: number) => {
      setPitch((prevPitch) => {
  ...
  useEffect(() => {
    const camera =
      cameras?.filter((item) => item.type == cameraType)[0] || null;
    console.log(camera);
  ...
  useEffect(() => {
    const Timer = setInterval(() => {
      ...
      if (webSocketRef.current) {
        console.log("zoonRef.current", zoomRef.current);
        console.log("prevZoomRef.current", prevZoomRef.current);
        console.log(
          "zoom abs diff",
          Math.abs(zoomRef.current - prevZoomRef.current)
        );
        if (
          currentCamera?.is_zoomable &&
          Math.abs(zoomRef.current - prevZoomRef.current) > 0.1
        ) {
        ...
        if (
          Math.abs(pitchRef.current - prevPitchRef.current) > 0.1 ||
          Math.abs(yawRef.current - prevYawRef.current) > 0.1
        ) {
          console.log(
            "pitch",
            (pitchRef.current - prevPitchRef.current).toFixed(1),
            "yaw",
            (yawRef.current - prevYawRef.current).toFixed(1)
          );
          webSocketRef.current.send(
            JSON.stringify({
              tid: generateUUID(),
      ...
      <div className="flex flex-col items-center space-y-2">
        ...
        <div className="text-xs text-gray-500">
          {cameras && (
            ...
            >
              {cameras?.map((camera) => (
                <option key={camera.type} value={camera.type}>
                  {camera.label}
                </option>
              ))}
            </select>
        ...
        <div className="mt-2">
          <button
            onClick={() => {
              if (webSocketRef.current) {
                setWorkingMode(workingMode == "manual" ? "auto" : "manual");
                webSocketRef.current.send(
                  JSON.stringify({
                    tid: generateUUID(),
                    timestamp: Math.floor(Date.now() / 1000),
                    method: "auto_mode",
                    data: {
                      action: workingMode == "manual" ? "start" : "stop",
                    },
                  })
                );
              }
            }}
            className="px-3 py-1 text-xs font-medium text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
            {workingMode == "manual" ? "自动模式" : "手动模式"}
          </button>
        </div>
      </div>
